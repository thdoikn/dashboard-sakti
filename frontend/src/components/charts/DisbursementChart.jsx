      ...
      {payload.map((entry) => (
        <div key={entry.name} className="flex items-center justify-between gap-4 py-0.5">
          ...
          <span className="font-semibold text-ikn-dark text-xs">
            {entry.name === '% Serapan' ? `${Number(entry.value).toFixed(1)}%` : formatIDR(entry.value)}
          </span>
        </div>
      ))}
    </div>
      ...
      <ComposedChart data={data} margin={{ top: 8, right: 20, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="realisasiGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="100%" stopColor={COLOR_REALISASI} stopOpacity={0.7} />
          </linearGradient>
        </defs>
        ...
        <Bar dataKey="anggaran"  name="Anggaran"   fill={COLOR_ANGGARAN}  radius={[4,4,0,0]} maxBarSize={32} />
        <Bar dataKey="realisasi" name="Realisasi"  fill="url(#realisasiGrad)" radius={[4,4,0,0]} maxBarSize={32} />
        <Line
